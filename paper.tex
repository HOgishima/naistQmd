...
\addbibresource{references/bibliography-jp.bib}
\addbibresource{references/bibliography-en.bib}
\usepackage{bookmark}
...
過去における研究としては\autocite{alex_nips12}などがある。Figure ~\ref{fig-CNN}
に示すように、CNNの構造は複雑である。

数式の表示を確認するため、いくつかの例を示す。インライン数式の例として、相関係数は
\(r = 0.85\) であった。また、平均値は \(\mu = 100\)、標準偏差は
\(\sigma = 15\) である。
...
は数学において最も美しい公式の一つとして知られている。

また、zスコアの計算式は Equation~\ref{eq-zscore} で示される：
...
Figure ~\ref{fig-muller-lyer}
は、ミュラー・リヤー錯視の典型的な例を示している。
...
Table ~\ref{tbl-descriptive} に、Big Five因子の記述統計量を示す。

\begin{table}

\caption{\label{tbl-descriptive}Big Five因子の記述統計量}
...
神経症傾向の分布を Figure ~\ref{fig-histogram} に示す。

\begin{figure}[htbp]

\caption{\label{fig-histogram}神経症傾向のヒストグラム}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{paper_files/figure-pdf/fig-histogram-1.pdf}}
...
\emph{p} = 0.00 , \emph{d} = 0.27,95 \%CI {[}0.19, 0.35{]})。この結果を
Figure ~\ref{fig-figs} に示す。

\begin{figure}

\caption{\label{fig-figs}神経症傾向の平均と標準誤差}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{paper_files/figure-pdf/fig-figs-1.pdf}}
...
\caption{\label{tbl-contingency}性別と教育歴についてのクロス集計表}
...
\emph{N} = 2800) = 8.61, \emph{p} = 0.00)。詳細は
Table ~\ref{tbl-contingency} を参照されたい。
...
\subsection{おまけその１}\label{sec-omake1}
...
\caption{\label{fig-omake}おまけの図}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{paper_files/figure-pdf/fig-omake-1.pdf}}

}

\end{figure}%

\subsection{おまけその２}\label{ux304aux307eux3051ux305dux306euxff12}
